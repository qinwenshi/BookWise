...
const getMetadata = (opf) => {
  ...
  const rendition = Object.fromEntries(
    getMetasByPrefix('rendition:').map(([k, el]) => [k, getElementText(el)])
  )
  ...
  for (const [k, el] of getMetasByPrefix('media:')) {
    ...
    if (k === 'duration')
      media.duration[el.getAttribute('refines')?.split('#')?.[1] ?? ''] = parseClock(v)
  ...
  async init() {
    ...
    this.metadata = {
      ...
      subject: metadata?.subject
        ...
        ?.map(({ value, term, authority }) => ({ name: value, code: term, scheme: authority })),
